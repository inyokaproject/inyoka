...
from django.contrib.contenttypes.fields import GenericForeignKey, GenericRelation, ReverseGenericRelatedObjectsDescriptor
...
class Content(models.Model):
    """
    Generic content field for other models that support InyokaMarkup and
    revisions.
    """
    # TODO: I could not give a variable application attribute to
    # InyokaMarkupField. So at the moment text is allways rendered with
    # application = 'portal' and the cache_key starts with 'portal'. The second
    # can be fixed.
    text = InyokaMarkupField()
    content_type = models.ForeignKey(ContentType)
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')
    user = models.ForeignKey(User, related_name='contents', null=True, blank=True)
    ...
    def restore(self):
        """
        Creates a new revision with the same content.
        """
        self.pk = None
        self.save()


class ContentField(GenericRelation):
    def __init__(self, *args, **kwargs):
        self.application = kwargs.pop('application', None)
        super(ContentField, self).__init__(Content, *args, **kwargs)

    def contribute_to_class(self, cls, name):
        """
        Adds custom attributes to the model.

        instance.get_FIELDNAME_rendered and instance.FIELDNAME_rendered are
        shutcuts for instance.latest().get_FIELDNAME_rendered and
        FIELDNAME_rendered to the model.

        instance.FIELDNAME_revisions is the RelatedManager generated by
        GenericRelation.

        instance.FIELDNAME is a object which setter and getter can be used
        to create to Content instances.
        """
        super(ContentField, self).contribute_to_class(
            cls,
            u"{}_revisions".format(name))

        @staticmethod
        def get_field_rendered(*args, **kwargs):
            return Context.get_text_rendered(*args, **kwargs)

        @property
        def field_rendered(inst_self):
            return getattr(inst_self, u"{}_revisions".format(name)).latest().text_rendered

        @property
        def field(inst_self):
            return getattr(inst_self, name).latest().text

        setattr(cls, 'get_{}_rendered'.format(name), get_field_rendered)
        setattr(cls, '{}_rendered'.format(name), field_rendered)

        setattr(cls, name, ContentReverseObjectDesciptor(self, self.for_concrete_model))


class ContentReverseObjectDesciptor(ReverseGenericRelatedObjectsDescriptor):
    """
    Represence Model.text when Model.text is a ContentField.

    This behaves like the parent class, but does also support instance.text =
    "str".
    """
    def __get__(self, instance, instance_type=None):
        try:
            return (super(ContentReverseObjectDesciptor, self)
    ...
    def __set__(self, instance, value):
        ...
        Value is a string.
        """
        # TODO: support setting of an author
        Content.objects.create(text=value, content_object=instance)
