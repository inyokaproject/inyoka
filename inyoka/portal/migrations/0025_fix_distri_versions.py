...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        distri_versions = orm.Storage.objects.get(key=KEY).value
        data = simplejson.loads(distri_versions)
        for distri in data:
            for k,v in distri.items():
                if v in ('false', 'true'):
                    distri[k] = True if v == 'true' else False
        distri_versions = simplejson.dumps(data)
        orm.Storage.objects.filter(key=KEY).update(value=distri_versions)
