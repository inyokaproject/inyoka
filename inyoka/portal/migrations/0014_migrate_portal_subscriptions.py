...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for subscription in orm.Subscription.objects.iterator():
            if subscription.topic_id is not None:
                app, model = ('forum', 'topic')
                object_id = subscription.topic_id
            elif subscription.forum_id is not None:
                app, model = ('forum', 'forum')
                object_id = subscription.forum_id
            elif subscription.ubuntu_version is not None:
                model = None
            elif subscription.wiki_page_id is not None:
                app, model = ('wiki', 'page')
                object_id = subscription.wiki_page_id
            elif subscription.member_id is not None:
                app, model = ('portal', 'user')
                object_id = subscription.member_id
            elif subscription.article_id is not None:
                app, model = ('ikhaya', 'article')
                object_id = subscription.article_id

            if not db.dry_run:
                if model is not None:
                    ctype = orm['contenttypes.ContentType'].objects.get(app_label=app, model=model)
                    subscription.content_type = ctype
                    subscription.object_id = object_id
                    subscription.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        sub = orm.Subscription.objects.get_for_model

        for subscription in orm.Subscription.objects.iterator():
            converters = {sub(Topic): 'topic_id',
                          sub(Forum): 'forum_id',
                          sub(Page): 'wiki_page_id',
                          sub(User): 'member_id',
                          sub(Article): 'article_id'}
            ctype = subscription.content_type

            for model_type, column in converters.iteritems():
                if ctype == model_type:
                    setattr(subscription, column, subscription.object_id)
