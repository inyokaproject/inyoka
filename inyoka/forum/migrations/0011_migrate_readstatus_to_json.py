...
def _to_list(iter):
    ...
    for item in iter:
        if isinstance(item, set):
            result.append(_to_list(list(item)))
        else:
            result.append(item)
    return result
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for user in orm['portal.User'].objects.all():
            if user.forum_read_status:
                status = cPickle.loads(user.forum_read_status.encode('utf-8'))
                status = {key: _to_list(value) for key, value in status.iteritems()}
                user.forum_read_status = json.dumps(status)
    ...
    def backwards(self, orm):
        ...
        for user in orm['portal.User'].objects.all():
            if user.forum_read_status:
                user.forum_read_status = cPickle.dumps(json.loads(user.forum_read_status))
                user.save(update_fields=('forum_read_status',))
