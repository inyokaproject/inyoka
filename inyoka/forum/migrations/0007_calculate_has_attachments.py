...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        post_ids = orm.Attachment.objects.values_list('post_id', flat=True).distinct()
        while post_ids:
            update = post_ids[:30]
            post_ids = post_ids[30:]
            orm.Post.objects.filter(pk__in=update).update(has_attachments=True)

    def backwards(self, orm):
        orm.Post.objects.update(has_attachments=False)
