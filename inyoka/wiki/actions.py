...
from inyoka.utils.http import (
    AccessDeniedResponse, TemplateResponse, does_not_exist_is_404, templated,
)
...
from inyoka.wiki.forms import (
    AddAttachmentForm, EditAttachmentForm, ManageDiscussionForm,
    MvBaustelleForm, PageEditForm,
)
        ...
        if form.is_valid():
            ...
            try:
                ...
                if not _rename(request, page, data['new_name'],
                               new_text=new_text):
                    messages.error(request,
                        u'Beim Verschieben in die Baustelle ist ein Fehler '
                        u'aufgereten.')
                    return HttpResponseRedirect(url_for(page))
            else:
                messages.error(request,
                    u'In der Baustelle befindet sich bereits eine Seite '
                    u'mit dem Namen „%s”.' % data['new_name'])
                return HttpResponseRedirect(url_for(page))
...
def do_mv_discontinued(request, name):
    ...
    if request.method == 'POST':
        if 'cancel' in request.POST:
            messages.info(request, u'Verschieben wurde abgebrochen.')
        else:
            ...
            try:
                ...
                return HttpResponseRedirect(url_for(page))
            messages.success(request,
                u'Seite wurde erfolgreich verschoben.')
    else:
        messages.info(request,
