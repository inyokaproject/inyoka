...
(function($) {
    ...
    var insert = function(format, def) {
        return function(evt) {
            return this.insertTag(format, (typeof def == 'undefined') ? 'Formatierter Text' : def);
        };
    };
    ...
    var toolbar = function() {
        return [
            button('bold', 'Fetter Text', insert("'''%s'''"), ['wiki', 'forum', 'small'], help("'''Text'''")),
            button('italic', 'Kursiver Text', insert("''%s''"), ['wiki', 'forum', 'small'], help("''Text''")),
            button('underlined', 'Unterstrichener Text', insert('__%s__'), ['forum', 'small'], help("__Text__")),
            button('stroke', 'Durchgestrichener Text', insert('--(%s)--'), ['forum'], help("--(Text)--")),
            button('monospace', 'Monotype', insert('`%s`'), ['wiki'], help("`Text`")),
            button('mark', 'Hervorgehobener Text', insert('[mark]%s[/mark]'), ['forum'], help("[mark]Text[/mark]")),
            button('wiki-link', 'Wiki-Link', insert('[:%s:]', 'Seitenname'), ['wiki', 'forum'], help("[:Seitenname:(Linktext)]")),
            button('external-link', 'Externer Link', insert('[%s]',
                'http://www.example.org/'), ['wiki', 'forum', 'small'], help("[www.example.com]")),
            button('user-link', 'Benutzerlink', insert('[user:%s:]', 'Benutzername'), ['forum'], help("[user:Beispiel:]")),
            button('quote', 'Auswahl zitieren', function(evt) {
                var selection = this.getSelection();
                if (selection.length)
                    this.setSelection(this.quoteText(selection));
            }, ['forum'], help("Auswahl zitieren")),
            button('picture', 'Bild', insert('[[Bild(%s)]]', 'Bildname'), ['wiki', 'forum'], help("[[Bild(Bildname)]]")), (function(editor) {
    ...
    WikiEditor.prototype.getCurrentLine = function() {
        ...
        if (typeof t.selectionStart != 'undefined') {
            var buffer = [];
            for (i = t.selectionEnd - 1;
                (c = t.value.charAt(i)) != '\n' && c; i--)
                buffer.push(c);
            buffer.reverse();
            for (i = t.selectionEnd;
                (c = t.value.charAt(i)) != '\n' && c; i++)
                buffer.push(c);
            return buffer.join('');
        }
