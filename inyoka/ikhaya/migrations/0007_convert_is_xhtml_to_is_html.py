...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for article in orm['ikhaya.Article'].objects.only('is_html', 'is_xhtml').all():
            article.is_html = article.is_xhtml
            article.save(update_fields=('is_html',))

    def backwards(self, orm):
        "Write your backwards methods here."
        for article in orm['ikhaya.Article'].objects.only('is_html', 'is_xhtml').all():
            article.is_html = article.is_xhtml
            article.save(update_fields=('is_xhtml',))
