...
"""
    ...
    Although the software is based on django we use (sometimes) different
    idoms and a non standard template engine Jinja. The basic
    ...
    The code is mostly tested with PostgreSQL but should work with SQLite or
    any other database Django supports.

    For the planet application the feedparser library must be installed.
    Additionally chardet is recommended so that it can better guess broken
    encodings of feeds. For the pastebin, wiki and some other parts `pygments`
    must be available. For incoming HTML data that is converted to XHTML we
    also need html5lib.

    We're using the recent LTS django releases.
    ...
    The configuration for development, test and production
    should start import `inyoka.default_settings`.
    ...
    `planet`
        A planet like feed aggregator. It has archives and santized
