...
class TestUserSha1PasswordRemoval(MigratorTestCase):
    ...
    def prepare(self):
        ...
        User.objects.create(
            username='foo2', email='foo2@local.localhost', password='sha1$not_valid'
        )

        User.objects.create(
            username='test',
            email='test@local.localhost',
            password='argon2$argon2id$v=19$m=102400,t=2,p=invalid',
        )
        User.objects.create(
            username='test2',
            email='test2@local.localhost',
            password='pbkdf2_sha256$600000$not_valid',
        )
...
class TestEventDateTimeMerge(MigratorTestCase):
    ...
    def prepare(self):
        ...
        only_start_date = event_model.objects.create(
            name='foo', slug='foo', date=date(2000, 1, 1), author=self.author
        )
        self.only_start_date_slug = only_start_date.slug

        only_start_date_and_time = event_model(
            name='foobar',
            slug='foobar',
            date=date(2001, 1, 1),
            time=time(1, 11),
...
class TestAdjustDateTime(MigratorTestCase):
    ...
    def test_dates(self):
        ...
        self.assertEqual(
            user.date_joined, datetime(2000, 8, 1, 15, 37, tzinfo=timezone.utc)
        )
